...
NODE_LABELS = {
    ...
    "grade_evidence": "判断证据是否足够",
    "answer_with_evidence": "融合证据生成回答",
...
ANSWER_NODES = {"ask_clarification", "answer_direct", "answer_with_evidence", "answer_with_limits"}
...
class GrowthRAGGraph:
    ...
    def _build_graph(self, checkpointer: Any | None = None):
        ...
        builder.add_edge("retrieve_records", "build_evidence_pack")
        builder.add_edge("build_evidence_pack", "grade_evidence")
        builder.add_conditional_edges(
            "grade_evidence",
            self._after_grade,
            {
    ...
    async def _checkpoint_config(
        ...
        expired_thread_ids = await asyncio.to_thread(self.memory_store.take_expired_checkpoint_threads)
    ...
    @staticmethod
    def _after_grade(state: GrowthRAGState) -> str:
        if state["evidence_grade"] and state["evidence_grade"]["is_sufficient"]:
    ...
    async def stream(self, payload: dict[str, Any]) -> AsyncIterator[StreamEvent]:
        ...
        graph = self._build_graph(checkpointer) if checkpointer is not None else self._fallback_graph
        ...
        while True:
            ...
            if graph_node != next_node:
                raise RuntimeError(f"Unexpected LangGraph node: expected {next_node}, got {graph_node}")
            ...
            elif graph_node == "grade_evidence" and state["evidence_grade"]:
                yield StreamEvent("evidence_grade", state["evidence_grade"])

            if graph_node in ANSWER_NODES:
                if graph_node in {"answer_with_evidence", "answer_with_limits"}:
    ...
    def _next_node(self, current: str, state: dict[str, Any]) -> str:
        ...
        if current == "grade_evidence":
            return self._after_grade(state)  # type: ignore[arg-type]
        if current in {"answer_with_evidence", "answer_with_limits"}:
            return "update_memory"
